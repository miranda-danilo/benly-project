...
// ---- LIMITADOR SIMPLE POR INSTANCIA ----
let lastCall = 0;
...
exports.handler = async (event) => {
    ...
    // ---- ANTI-SPAM SIMPLE (solo por instancia) ----
    const now = Date.now();
    const diff = now - lastCall;
    if (diff < 500) { // max 2 requests por segundo por instancia
        await new Promise(res => setTimeout(res, 500 - diff));
    }
    lastCall = Date.now();
    ...
    const client = new GoogleGenerativeAI(apiKey);
    const model = client.getGenerativeModel({
        model: "gemini-2.5-flash-live", // ✅ Modelo live estable
        generationConfig: {
            responseMimeType: "application/json"
        },
        safetySettings: [
    ...
    // ---- Función de generación con retry exponencial ----
...
`;
            ...
            if (retries > 0) {
                await new Promise(res => setTimeout(res, delay));
                return generateWithRetry(retries - 1, delay * 2); // delay exponencial
            }
            throw err;
        }
    }
